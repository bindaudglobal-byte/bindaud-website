import { getOrdersAsync } from "./adminStorage.js";

const select = (selector, parent = document) => parent.querySelector(selector);

const findMatchingOrders = async (query) => {
...
const renderResults = (container, orders) => {
  ...
  if (!orders.length) {
    container.innerHTML =
      '<p class="empty-state">No orders matched your search yet. Try your phone number or order reference.</p>';
    return;
  }
...
export const initOrderTracking = () => {
  ...
  const showLatest = async () => {
    ...
    if (fallback) {
      fallback.innerHTML = latestOrders
        .map(
          (order) => `
  ...
  form.addEventListener("submit", async (event) => {
    ...
    renderResults(results, await findMatchingOrders(query));
  });
};
