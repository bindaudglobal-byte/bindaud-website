...
const handleImagePreview = (fileInput) => {
  ...
  reader.onload = (e) => {
    let existingPreview =
      fileInput.parentElement.querySelector(".image-preview");
    if (!existingPreview) {
...
const populateProductForm = (product = null) => {
  ...
  if (product?.image && !product.image.startsWith("data:")) {
    ...
    select("#product-image-file", form).parentElement.insertBefore(
      previewDiv,
      select("#product-image-file", form).nextSibling,
    );
  }

  select("#product-form-submit", form).textContent = product
    ? "Update Product"
    : "Add Product";
};
...
export const initAdminDashboard = async () => {
  ...
  productForm?.addEventListener("submit", async (event) => {
    ...
    const payload = {
      ...
      sizeOptions: (formData.get("sizes") || "")
        .split(",")
        .map((size) => size.trim())
        .filter(Boolean),
      colorOptions: (formData.get("colors") || "")
        .split(",")
