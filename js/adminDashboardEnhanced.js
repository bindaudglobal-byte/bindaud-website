...
const exportAdminData = () => {
  ...
  setTimeout(() => {
    const state =
      window.__BINDAUD_ADMIN_STATE ||
      window.__BINDAUD_STORAGE__?.bindaud_admin_state;
    if (!state) {
...
const importAdminData = async (file) => {
  ...
  try {
    ...
    window.__BINDAUD_ADMIN_STATE = data;
    window.__BINDAUD_STORAGE__ = {
      ...(window.__BINDAUD_STORAGE__ || {}),
      bindaud_admin_state: data,
    };
...
const saveProductDraft = () => {
  ...
  window.__BINDAUD_PRODUCT_DRAFT = draft;
};

const loadProductDraft = () => {
  const draft = window.__BINDAUD_PRODUCT_DRAFT;
...
const clearProductDraft = () => {
  window.__BINDAUD_PRODUCT_DRAFT = null;
};
...
const getFilteredOrders = async () => {
  ...
  const searchValue = (select("#orders-search")?.value || "")
    .trim()
    .toLowerCase();
...
const renderRecentOrders = async () => {
  ...
  if (!orders.length) {
    container.innerHTML =
      '<p class="admin-empty">No orders have been submitted yet.</p>';
    return;
  }
...
const populateProductForm = (product = null) => {
  ...
  select("#visibility-toggle", form).checked = Boolean(
    product?.visibility === "Hidden",
  );

  // SEO fields
  select("#product-seo-title", form).value = product?.seoTitle || "";
  select("#product-meta-description", form).value =
    ...
    product?.images && Array.isArray(product.images) && product.images.length
      ...
      : product?.image
        ? [
            {
              url: product.image,
              name: `${product.name || "Product"} cover`,
              isCover: true,
            },
          ]
        : [];
...
const getFilteredProducts = (products) => {
  const searchValue = (select("#product-search")?.value || "")
    .trim()
    .toLowerCase();
  ...
  if (visibilityValue !== "all") {
    filtered = filtered.filter(
      (product) => product.visibility === visibilityValue,
    );
  }
...
const renderProducts = async () => {
  ...
  list.innerHTML = pageProducts
    .map((product) => {
      ...
      const coverUrl = imageUrl.startsWith("data:")
        ? imageUrl
        : resolveSitePath(imageUrl);
...
const renderOrders = async () => {
  ...
  if (!filteredOrders.length) {
    tableBody.innerHTML =
      '<tr><td colspan="7" class="admin-empty">No matching orders found.</td></tr>';
    return;
  }
...
const renderWebsiteSettings = () => {
  ...
  select("#tax-enabled", form).checked = settings.taxEnabled !== false;
...
const renderAnalytics = () => {
  ...
  topSelling.innerHTML = analytics.topSelling
    .map(
      (product) => `
  ...
  mostViewed.innerHTML = analytics.mostViewed
    .map(
      (product) => `
    <li class="admin-list-item">
      <strong>${product.name}</strong>
  ...
  revenueChart.innerHTML = analytics.topSelling
    .slice(0, 4)
    .map(
      (product) => `
    <div class="chart-bar">
  ...
  activityFeed.innerHTML = analytics.recentActivity
    .map(
      (activity) => `
    <li class="admin-list-item">
...
export const initAdminDashboard = async () => {
  ...
  duplicateBtn?.addEventListener("click", async (e) => {
    ...
    const duplicated = ProductDuplicator.duplicate(
      currentEditingProduct,
      `${currentEditingProduct.name} (Copy)`,
    );
  ...
  productForm?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      let imageValue =
        formData.get("imagePath") || "assets/products/product1.jpg";

      const uploadedImage = imageUploader?.getFile();
      if (
        uploadedImage &&
        typeof uploadedImage === "object" &&
        "url" in uploadedImage
      ) {
      ...
      const visibilityState = select("#visibility-toggle")?.checked
        ? "Hidden"
        : "Public";
      const slugValue =
        select("#product-slug")?.value || generateSlugFromName(nameValue);
  ...
  document.addEventListener("click", async (event) => {
    ...
    if (
      event.target.id === "product-page-prev" ||
      event.target.id === "product-page-next"
    ) {
      const products = await getProducts();
      const filteredProducts = getFilteredProducts(products);
      const totalPages = Math.max(
  ...
  ordersSort?.addEventListener("change", async () => {
    await renderOrders();
  });
  ...
  deleteSelectedProductsBtn?.addEventListener("click", async () => {
    ...
    await Promise.all(
      Array.from(selectedProductIds).map((id) => deleteProduct(id)),
    );
  ...
  websiteForm?.addEventListener("submit", (event) => {
    ...
    const settings = {
      ...
      taxEnabled: select("#tax-enabled", websiteForm)?.checked !== false,
