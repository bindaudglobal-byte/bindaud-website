...
export const initProductionAdminDashboard = async () => {
  ...
  try {
    const token = window.__BINDAUD_ADMIN_TOKEN || "";
    ...
    if (Array.isArray(products) && products.length > 0) {
      ...
      container.innerHTML =
        '<p class="admin-empty">No products available. Create your first product!</p>';
    }
  } catch (error) {
...
const renderProductsFromBackend = (products) => {
  ...
  container.innerHTML = products
    .map((product) => {
      const imageUrl =
        product.images?.[0]?.url || "assets/products/product1.jpg";
...
const editProductFromBackend = async (productId) => {
  try {
    ...
    if (form) {
      ...
      form.querySelector('[name="sizes"]').value = Array.isArray(product.sizes)
        ? product.sizes.join(", ")
        : "";
      form.querySelector('[name="colors"]').value = Array.isArray(
        product.colors,
      )
        ? product.colors.join(", ")
        : "";
      form.querySelector('[name="stock"]').value = product.stock || 0;
      form.querySelector('[name="featured"]').checked = Boolean(
        product.featured,
      );
      form.querySelector('[name="bestSeller"]').checked = Boolean(
        product.bestSeller,
      );
