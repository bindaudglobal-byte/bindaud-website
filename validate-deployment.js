...
addCheck(
  ...
  fileExists("backend/services/emailService.js"),
  "",
);
...
const normalizeRouteCheck = (source, method, route) => {
  const singleQuotePattern = new RegExp(`router\\.${method}\\('" + route + "'`);
  const doubleQuotePattern = new RegExp(`router\\.${method}\\(\"${route}\"`);
  const regex = new RegExp(`router\\.${method}\\(['\"]${route}['\"]`);
  return regex.test(source);
};

addCheck(
  "POST /api/admin/orders endpoint",
  normalizeRouteCheck(adminRoutes, "post", "/orders"),
  "Public order creation",
);
addCheck(
  "GET /api/admin/orders endpoint",
  normalizeRouteCheck(adminRoutes, "get", "/orders"),
  "Fetch all orders",
);
addCheck(
  "PUT /api/admin/orders/:id endpoint",
  normalizeRouteCheck(adminRoutes, "put", "/orders/:id"),
  "Update order status",
);
addCheck(
  "POST /api/admin/email endpoint",
  normalizeRouteCheck(adminRoutes, "post", "/email"),
...
addCheck(
  ...
  emailJs.includes("sendEmailViaBackend"),
  "Backend API call added",
);
addCheck(
  "Email async/await",
  emailJs.includes("async"),
  "Async email functions",
);

const cartJs = readFile("js/cart.js");
addCheck(
  "Cart backend order POST",
  cartJs.includes("createOrderAsync(") ||
    cartJs.includes("fetch('/api/admin/orders'") ||
    cartJs.includes('requestAdminApi("/orders"') ||
    cartJs.includes("requestAdminApi('/orders')"),
  "Order POST to backend",
);
addCheck(
  "Cart email awaiting",
  cartJs.includes("await queueOrderEmail"),
  "Email async handling",
);

const adminDashboard = readFile("js/adminDashboard.js");
addCheck(
  "Admin async renderOrders",
  adminDashboard.includes("async () =>") &&
    adminDashboard.includes("await renderOrders()"),
  "Event listeners async",
);
addCheck(
  "Admin getOrdersAsync",
  adminDashboard.includes("getOrdersAsync"),
  "Backend order fetching",
);

const cartStorage = readFile("js/cartStorage.js");
addCheck(
  "Tax getTaxRate function",
  cartStorage.includes("getTaxRate()"),
  "Tax configuration",
);
addCheck(
  "Tax calculation updated",
  cartStorage.includes("taxableBase * taxRate") ||
    cartStorage.includes("(subtotal - discountAmount + shipping) * taxRate"),
  "Tax formula",
);
...
addCheck(
  ...
  adminCss.includes("max-width: 768px"),
  "",
);
addCheck(
  "Mobile breakpoint (480px)",
  adminCss.includes("max-width: 480px"),
  "",
);
addCheck(
  "Toolbar responsive styles",
  adminCss.includes(".admin-toolbar-row"),
  "",
);
...
addCheck(
  "MONGODB_URI in example",
  envExample.includes("MONGODB_URI"),
  "Database connection",
);
addCheck(
  "JWT_SECRET in example",
  envExample.includes("JWT_SECRET"),
  "Authentication",
);
addCheck(
  "EMAIL config in example",
  envExample.includes("EMAIL_HOST"),
  "Email service",
);
